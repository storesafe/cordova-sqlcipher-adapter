...
var scenarioList = [
  ...
  'HTML5',
  // XXX TBD SKIP FOR NOW (see below):
  // 'Plugin-implementation-2'
];

// XXX TBD SKIP `androidDatabaseProvider: 'system'`
// (`androidDatabaseImplementation: 2`) for now
// to avoid a few test failures
// var scenarioCount = (!!window.hasWebKitWebSQL) ? (isAndroid ? 3 : 2) : 1;
var scenarioCount = (!!window.hasWebKitWebSQL) ? 2 : 1;
  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': tx string test(s)', function() {
      ...
      describe(suiteName + 'U+0000 character tests', function() {
        ...
        it(suiteName + 'String HEX encoding test with U+0000 character (same as \\0 [null]) [XXX consistent encoding on Android in this plugin version, TRUNCATION ISSUE REPRODUCED on Windows; default sqlite encoding: UTF-16le on Windows & Android 4.1-4.4 (WebKit) Web SQL, UTF-8 otherwise]', function (done) {
          ...
          db.transaction(function (tx) {
            tx.executeSql('SELECT HEX(?) AS hexvalue', ['abcd'], function (tx_ignored, rs1) {
              ...
              tx.executeSql('SELECT HEX(?) AS hexvalue', ['a\u0000cd'], function (tx_ignored, rs2) {
                ...
                // XXX NO UTF-8 with XXX ENCODING BUG on Android
                // in this plugin version
                // else if (!isWebSql && isAndroid && !isImpl2)
                //   expect(resultRow2.hexvalue).toBe('61C0806364'); //  (XXX ...)
                else
                ...
                //   expect(resultRow2.hexvalue.length).toBe(10);  // (XXX ...)
                else
                ...
                tx.executeSql('SELECT HEX(?) AS hexvalue', ['e\0gh'], function (tx_ignored, rs3) {
                  ...
                  // in this plugin version
                  // else if (!isWebSql && isAndroid && !isImpl2)
                  //   expect(resultRow3.hexvalue).toBe('65C0806768'); // (XXX ...)
                  else
        ...
        it(suiteName + 'INLINE HEX encoding test with U+0000 (\\0) [XXX SQL ERROR ALWAYS reported; default sqlite encoding: UTF-16le on TBD, UTF-8 otherwise]', function (done) {
      ...
      describe(suiteName + 'UTF-8 multi-byte character string binding & manipulation tests', function() {
        ...
        it(suiteName + "SELECT LOWER(X'41EDA080EDBCB1') - result column value is '\\uED41\u80A0\\uBCED' ('\uED41\u80A0\uBCED') on Android 4.1-4.3 (WebKit) Web SQL & Windows (UTF-16le), 'a\uD800\uDF31' XXX MISSING on iOS/macOS plugin, XXX 'a\\uFFFD\\uFFFD' ('a\uFFFD\uFFFD') on Android & (WebKit) Web SQL (Android post-4.3/iOS/Browser)", function(done) {
          // ref: litehelpers/Cordova-sqlite-storage#564
        ...
        it(suiteName + 'emoji HEX test: SELECT HEX("@\\uD83D\\uDE03!") [\\u1F603 SMILING FACE (MOUTH OPEN)] [XXX consistent encoding on Android in this plugin version; default sqlite HEX encoding: UTF-6le on Windows & Android 4.1-4.3 (WebKit) Web SQL, UTF-8 otherwise]', function(done) {
          // XXX NO BUG in this plugin version:
          // ref: litehelpers/Cordova-sqlite-storage#564
        ...
        it(suiteName + "SELECT LOWER(X'41EDA0BDEDB88321') - result column value is '\\uED41\\uBDA0\\uB8ED\\u2183' ('\uED41\uBDA0\uB8ED\u2183') on Android 4.1-4.3 (WebKit) Web SQL & Windows (UTF-16le), XXX MISSING on iOS/macOS plugin, '\\uED41\\uBDA0\\uB8ED\\u2183' ('\uED41\uBDA0\uB8ED\u2183') on Android & (WebKit) Web SQL (Android post-4.3/iOS/Browser)", function(done) {
          // ref: litehelpers/Cordova-sqlite-storage#564
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql("SELECT LOWER(X'41EDA0BDEDB88321') AS lowertext", [], function(ignored, rs) {
              ...
              else if (isWebSql || isAndroid) // XXX (...)
                expect(rs.rows.item(0).lowertext).toBe('a\uFFFD\uFFFD!'); // 'a��!'
      ...
      describe(suiteName + 'additional (extra) multi-byte UTF-8 character string binding & manipulation tests', function() {
        ...
        it(suiteName + 'string parameter manipulation test with a combination of UTF-8 2-byte & 3-byte characters', function(done) {
          // XXX TBD ???:
          // if (!isWebSql && isAndroid && isImpl2) pending('SKIP for androidDatabaseImplementation: 2 setting'); // QUICK TEST WORKAROUND
      ...
      //* ** NO LONGER DISABLED IN THIS VERSION BRANCH:
      ...
      describe(suiteName + 'string test with Array "subclass" for SQL parameter arg values array', function() {
        ...
        it(suiteName + 'SELECT UPPER(?) AS upper1, UPPER(?) AS upper2 with "naive" Array subclass (constructor explicitly set to subclasss) as value arguments array [SQL argument values IGNORED by plugin]', function(done) {
          if (!isWebSql && !isWindows && isAndroid) pending('SKIP for Android plugin'); // XXX TBD ???
